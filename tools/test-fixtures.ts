...
export async function fixtureDocx(text: string): Promise<Buffer> {
  const zip = new JSZip();
...
export async function fixtureXlsx(rows: string[][]): Promise<Buffer> {
  const zip = new JSZip();
  zip.file("[Content_Types].xml", `<?xml version="1.0" encoding="UTF-8"?>
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  <Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>
...
export function fixturePdf(text: string): Buffer {
...
export async function fixtureEmlWithDocxAttachment(text: string): Promise<string> {
  const docx = await fixtureDocx(text);
